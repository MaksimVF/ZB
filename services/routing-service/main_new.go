...
import (
	...
	"github.com/MaksimVF/ZB/services/routing-service/grpc"
	"github.com/MaksimVF/ZB/services/routing-service/monitoring"
	"github.com/MaksimVF/ZB/services/routing-service/circuit"
...
func main() {
	...
	cache := storage.NewLRUCache(cfg.Cache.MaxSize, cfg.Cache.TTL)
	...
	circuitBreaker := circuit.NewCircuitBreaker(cfg)
	...
	grpcHandlers := grpc.NewGRPCHandlers(routingEngine, registry, &InMemoryPolicyManager{policy: policy})
...
func startGRPCServer(grpcHandlers *grpc.GRPCHandlers, cfg *config.Config, logger *zap.Logger) {
...
func startHTTPServer(
	...
	router.Put("/api/routing/policy", adminHandlers.RequireRole(httpMiddleware.RoleAdmin)(http.HandlerFunc(adminHandlers.UpdateRoutingPolicy)))
	router.Get("/api/routing/heads", adminHandlers.GetAllHeads)
	router.Post("/api/routing/heads", adminHandlers.RequireRole(httpMiddleware.RoleOperator)(http.HandlerFunc(adminHandlers.RegisterHead)))
