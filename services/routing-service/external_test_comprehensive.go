...
type ComprehensiveExternalTestSuite struct {
	suite.Suite
}

func (suite *ComprehensiveExternalTestSuite) SetupTest() {
	// Initialize the service for testing
	initService()
}

func (suite *ComprehensiveExternalTestSuite) TestWebhookEndpoints() {
...
func (suite *ComprehensiveExternalTestSuite) TestRateLimiter() {
...
func (suite *ComprehensiveExternalTestSuite) TestCircuitBreaker() {
...
func (suite *ComprehensiveExternalTestSuite) TestGraphQLAPI() {
...
func (suite *ComprehensiveExternalTestSuite) TestWebSocketEndpoints() {
...
func (suite *ComprehensiveExternalTestSuite) TestSSEEndpoints() {
	// Test SSE endpoint
...
func (suite *ComprehensiveExternalTestSuite) TestMessageQueueIntegration() {
...
func (suite *ComprehensiveExternalTestSuite) TestExternalServiceIntegration() {
...
func (suite *ComprehensiveExternalTestSuite) TestSecurityFeatures() {
	// Test JWT validation
...
func (suite *ComprehensiveExternalTestSuite) TestAdvancedRateLimiter() {
	// Test advanced rate limiter features
	suite.T().Run("AdvancedRateLimiter", func(t *testing.T) {
		ip := "192.168.1.3"

		// Set custom burst limit
		rateLimiter.SetBurstLimit(ip, 3)

		// Set custom reset timeout
		rateLimiter.SetResetTimeout(ip, 30*time.Second)

		// Set custom burst duration
		rateLimiter.SetBurstDuration(ip, 5*time.Second)
...
func (suite *ComprehensiveExternalTestSuite) TestAdvancedCircuitBreaker() {
	// Test advanced circuit breaker features
	suite.T().Run("AdvancedCircuitBreaker", func(t *testing.T) {
		service := "test-service-advanced"

		// Set custom threshold
		circuitBreaker.SetThreshold(service, 2)

		// Set custom reset timeout
		circuitBreaker.SetResetTimeout(service, 20*time.Second)

		// Set custom half-open duration
		circuitBreaker.SetHalfOpenDuration(service, 5*time.Second)
...
func (suite *ComprehensiveExternalTestSuite) TestComprehensiveMetrics() {
	// Test comprehensive metrics collection
	suite.T().Run("ComprehensiveMetrics", func(t *testing.T) {
		// Test rate limiter metrics
		ip := "192.168.1.4"
		rateLimiter.Allow(ip)
		rateLimiter.Allow(ip)

		rateMetrics := rateLimiter.Metrics()
		assert.Contains(t, rateMetrics, ip)
		assert.Equal(t, 2, rateMetrics[ip].(map[string]interface{})["requests"])

		// Test circuit breaker metrics
		service := "test-service-metrics"
		circuitBreaker.Fail(service)
		circuitBreaker.Fail(service)

		circuitMetrics := circuitBreaker.Metrics()
		assert.Contains(t, circuitMetrics, service)
		assert.Equal(t, 2, circuitMetrics[service].(map[string]interface{})["failures"])
	})
}

func (suite *ComprehensiveExternalTestSuite) TestComprehensiveRecovery() {
	// Test comprehensive recovery features
	suite.T().Run("ComprehensiveRecovery", func(t *testing.T) {
		// Test rate limiter recovery
		ip := "192.168.1.5"
		rateLimiter.SetRecoveryAttempts(ip, 3)
		rateLimiter.SetRecoverySuccesses(ip, 2)
		rateLimiter.SetRecoveryFailures(ip, 1)

		// Test circuit breaker recovery
		service := "test-service-recovery"
		circuitBreaker.SetRecoveryAttempts(service, 3)
		circuitBreaker.SetRecoverySuccesses(service, 2)
		circuitBreaker.SetRecoveryFailures(service, 1)

		// Test recovery metrics
		rateMetrics := rateLimiter.Metrics()
		assert.Contains(t, rateMetrics, ip)
		assert.Equal(t, 3, rateMetrics[ip].(map[string]interface{})["recovery_attempts"])

		circuitMetrics := circuitBreaker.Metrics()
		assert.Contains(t, circuitMetrics, service)
		assert.Equal(t, 3, circuitMetrics[service].(map[string]interface{})["recovery_attempts"])
	})
}

func TestComprehensiveExternalTestSuite(t *testing.T) {
	suite.Run(t, new(ComprehensiveExternalTestSuite))
}
